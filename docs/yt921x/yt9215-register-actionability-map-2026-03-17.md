...
| `0x220104/0x220108/0x220800` (`METER_TIMESLOT`/`PORT_METER_CTRL`/`METER_CFG`) and `0x34c000..` (`QSCH_SHAPER`) | Now live in driver subset: ingress `port_policer` uses `0xc7/0xc8/0xce`; queue `tc tbf` uses `0xe4/0xe9` and token path in `0xea`. Full stock parity for all `0xea` subfields is still incomplete. | Keep masked RMW. Safe to use through driver APIs (`tc`/DSA policer), avoid direct exploratory writes on unmapped subfields. |
...
| `0x300200/0x300280/0x300400`, `0x341000..0x343000`, `0x34c000..0x357000` (stock QoS schedule/shaper tables) | Stock reverse + symbol-field decode is now partially wired in driver: queue map (`mqprio`), scheduler safe subset (`ets` + mqprio baseline), queue/port tbf shaping paths. Remaining gap is full scheduler-policy parity and non-token meter semantics. |
