...
| `YT921X_QOS_REMARK_CPRI_SPRI` | `0x00100200` | dynamic / remark-default-mapped | Egress CPRI/SPRI remark table (`tbl id 0xdc`); stock CPRI/SPRI paths use fields `1` and `0` (paired with `0xd9:f7/f8`). | Medium |
...
| `YT921X_RATE_METER_CONFIGn(n)` (`0x220800+0x10*n`) | decoded fields across words 0/1/2 | set with field RMW per decoded map (`meter_config_tblm_field` entries 0..13) | current `port_policer` path uses this table (`tbl 0xce`), with ingress binding in upper bank (`n = meter_id + 64`) |
